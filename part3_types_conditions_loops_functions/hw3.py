...
def parse_amount(raw_number: str) -> float | None:
    ...
    if normalized_number.count(".") > 1 or normalized_number == "":
        ...
        if number_part == "":
            ...
            is_valid = (
                left_part != "" and right_part != "" and is_normal_number(left_part) and is_normal_number(right_part)
            )
        else:
...
def cost_categories_handler() -> str:
    ...
    for common_category, target_categories in EXPENSE_CATEGORIES.items():
        all_categories.extend(f"{common_category}{SEPARATOR}{target_category}" for target_category in target_categories)
...
def process_transaction_for_month(
    ...
    if transaction_date is None:
        return month_income, month_expenses
    if not (is_same_month(transaction_date, report_date_tuple) and is_not_after(transaction_date, report_date_tuple)):
        return month_income, month_expenses
